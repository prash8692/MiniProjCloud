...
then
	...
	echo "1. AMI ID, 2. Key-Name, 3. Security Group, 4. Launch Configuration, 5. count (count cannot be less than zero)"
	echo "If you are passing 7 parameter the parameter should of below order"
	echo "1. AMI ID, 2. Key-Name, 3. Security Group, 4. Launch Configuration, 5. count (count cannot be less than zero), 6. Clinet Token, 7. Auto scaling group name, 8.Load Balancer name"
	exit 0;
fi
...
echo "you have not passed exactly five/eight parameter";
exit 0;
fi
